...
#' and "]"). It then checks for hyphens "-", underscores "_", and single spaces
#' " ". If these are found, variants of the name with the
#' hyphen/underscore/spaces replaced by each of the other two, as well as all
#' subnames spearated by these characters, and all subnames pasted together with
#' none of these special characters, are searched against tax2map2 for exact
#' matches. It also creates all-lower and all-upper case versions of these
#' elements and again searches for exact name matches in tax2map2. Words
#' generated by this process that are 2 characters or less are not searched for
#' matches in tax2map2. All alternative names created when ignore.format = TRUE
#' are also searched for synonyms in synonym.file if specified.
#'
